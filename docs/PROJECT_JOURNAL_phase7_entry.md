...
### Results
**PENDING.** See `docs/phase7-test-evaluation.md` (generated by running
`PYTHONPATH=. python -m ml.test_evaluation` against the real data) for the
actual TEST row count, positive rate, precision, recall, F1, ROC-AUC,
PR-AUC, and confusion matrix. Not fabricated here -- this section is
intentionally left as a placeholder until the real, one-time run happens.
